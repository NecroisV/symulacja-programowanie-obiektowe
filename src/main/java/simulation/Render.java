...
import event.Earthquake;
import event.Event;
import event.Fog;
import event.Thunderstorm;
...
/**
 * Klasa odpowiedzialna za renderowanie (rysowanie) pełnego stanu symulacji na ekranie.
 * Wykorzystuje komponent {@link GraphicsContext} z biblioteki JavaFX do rysowania dwuwymiarowej
 * planszy, kafelków środowiska, agentów, interfejsu górnego (porady dnia, zdarzenia)
 * oraz dolnego panelu statystyk i logów tury.
 */
public class Render {

    /**
     * Główna metoda renderująca, wywoływana w każdym kroku pętli aplikacji w celu odświeżenia obrazu.
     * Rysuje kolejno: tło, górny panel informacyjny, siatkę świata (ściany, zasoby, strefy bezpieczeństwa),
     * agentów (ocalałych, zainfekowanych lub nakładających się na siebie agentów) oraz dolny panel z logami i statystykami.
     * * <p><b>Uwaga:</b> Metoda na koniec swojego działania czyści listę {@code turnLogs} (wywołuje {@code turnLogs.clear()}),
     * aby przygotować ją na wpisy z kolejnej tury.</p>
     *
     * @param gc           Obiekt {@link GraphicsContext} powiązany z płótnem (Canvas), na którym odbywa się rysowanie.
     * @param tileSize     Rozmiar pojedynczego kafelka planszy wyrażony w pikselach.
     * @param turnLogs     Lista ciągów znaków zawierająca logi zdarzeń z bieżącej tury (czyszczona na końcu metody).
     * @param data         Obiekt {@link DataCollector} przechowujący aktualne statystyki populacji i interakcji.
     * @param actualTick   Numer bieżącego kroku (ticka/tury) symulacji.
     * @param board        Dwuwymiarowa tablica obiektów {@link Space} reprezentująca aktualny stan siatki świata.
     * @param currentEvent Aktualnie aktywne zdarzenie środowiskowe ({@link Event}) lub {@code null}, jeśli brak zagrożeń.
     * @param timeOfDay    Tekstowy opis aktualnej pory dnia (np. "Dzień", "Noc").
     */
    void renderBoard(GraphicsContext gc, double tileSize, List<String> turnLogs, DataCollector data, int actualTick, Space[][] board, Event currentEvent, String timeOfDay) {
        ...
        // Czyszczenie i rysowanie głównego tła okna
        gc.setFill(Color.web("#1e1e24"));
        gc.fillRect(0, 0, gc.getCanvas().getWidth(), gc.getCanvas().getHeight());

        // Rysowanie tła górnego panelu
        ...
        // Rysowanie informacji o porze dnia
        ...
        // Rysowanie informacji o aktywnym evencie
        ...
        // Pętla rysująca siatkę kafelków (planszę)
        ...
        // RYSOWANIE DOLNEGO PANELU (STATYSTYKI I LOGI)
        ...
        // Wyświetlanie ostatnich wpisów z logów tury (maksymalnie 4)
        ...
        // Wyczyszczenie logów na koniec cyklu renderowania
        turnLogs.clear();

        // Rysowanie kontenerów Live Stats
        double statsStartX = Math.max(380, width * tileSize * 0.55);

        // Panel: LIVE STATS 1
        gc.setFill(Color.web("#25252d"));
        gc.fillRect(statsStartX - 10, panelStartY, 200, 150);
